<template>
    <div class="modal fade" id="godlyChestModal" tabindex="-1" role="dialog" style="display: none;" aria-hidden="true">
...
<script>
    export default {
        name: "GodlyChestModal",
        data: function() {return {loading: false, response: false, message: ''}},
        methods: {
            whatsInside: function () {
                if(this.loading === true) return false;
                if(this.response === true) {
                    $('#godlyChestModal').modal('hide');
                    this.$router.go();
                }

                $('#godlyChestModal').modal('show');

                this.loading = !this.loading;
                const user = $helper.getUser();
                const FORM_DATA = new FormData();
                FORM_DATA.append('type', 'legendary');
                ...
                $api.postData('open-chest', FORM_DATA)
                    .then((response) => {
                        if(response > 0) {
                            ...
                            $helper.saveLocalData('godly-chest', 1);
                        } else {
                            Swal.fire(
                                'Oops!',
