<template>
    <div class="modal fade" id="rareChestModal" tabindex="-1" role="dialog" style="display: none;" aria-hidden="true">
        ...
        methods: {
            whatsInside: function () {
                ...
                if(this.response === true) {
                    $('#rareChestModal').modal('hide');
                    this.$router.go();
                }

                $('#rareChestModal').modal('show');

                this.loading = !this.loading;
                const user = $helper.getUser();
                const FORM_DATA = new FormData();
                FORM_DATA.append('type', 'epic');
                ...
                $api.postData('open-chest', FORM_DATA)
                    .then((response) => {
                        if(response > 0) {
                            ...
                            $helper.saveLocalData('rare-chest', 1);
                        } else {
                            Swal.fire(
                                'Oops!',
